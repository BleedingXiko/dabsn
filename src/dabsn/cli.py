...
def _load_config(path: str | Path) -> DABSNConfig:
    return DABSNConfig(**_read_json(path))
...
def _doctor(args) -> int:
    config = _load_config(args.config) if args.config else DABSNConfig(
        input_dim=5,
        out_dim=3,
        layers=[
...
def _supervised_train(args, *, initial_checkpoint: str | None = None) -> int:
    ...
    state = setup_distributed(args.distributed, args.device)
    try:
        config = _load_config(args.config) if args.config else None
        output = Path(args.output)
        if initial_checkpoint is not None and Path(initial_checkpoint).resolve() == output.resolve():
        ...
        if args.resume:
            ...
            if args.checkpoint_mode == "portable" and not optimizer_checkpoint_path(output).is_file():
        ...
        if initial_checkpoint is not None:
            model = load_dabsn(initial_checkpoint, map_location=state.device)
            if args.grad_checkpoint is not None:
                model.backbone.grad_checkpoint = bool(args.grad_checkpoint)
        elif args.resume and args.checkpoint_mode == "portable":
            model = load_dabsn(output, map_location=state.device)
            if args.grad_checkpoint is not None:
                model.backbone.grad_checkpoint = bool(args.grad_checkpoint)
        elif args.resume:
            manifest = inspect_sharded_training_checkpoint(output)
            model = build_dabsn_from_checkpoint_config(manifest["config"]).to(state.device)
            if args.grad_checkpoint is not None:
                model.backbone.grad_checkpoint = bool(args.grad_checkpoint)
        else:
            if config is None:
                raise ValueError("new training requires --config")
            model = build_dabsn_from_config(
                config,
        ...
        parameter_groups = dabsn_adamw_param_groups(model, args.weight_decay)
        ...
        for step in range(start_step + 1, args.steps + 1):
            ...
            if scaler is None:
                ...
                scaler.update()
            if state.enabled:
                torch_dist.all_reduce(local_loss, op=torch_dist.ReduceOp.SUM)
                local_loss /= state.world_size
...
def _pretrain(args) -> int:
    config = DABSNPretrainConfig(**_read_json(args.config))
...
def _load_execution_model(args, state, precision: str):
    if args.checkpoint_mode == "portable":
        raw = load_dabsn(args.checkpoint, map_location=state.device)
        wrapped = prepare_distributed_model(raw, state, precision=precision)
        return raw, wrapped
    manifest = inspect_sharded_training_checkpoint(args.checkpoint)
    raw = build_dabsn_from_checkpoint_config(manifest["config"]).to(state.device)
    wrapped = prepare_distributed_model(raw, state, precision=precision)
...
def _evaluate(args) -> int:
    ...
    try:
        ...
        if state.enabled:
            torch_dist.all_reduce(loss, op=torch_dist.ReduceOp.SUM)
            loss /= state.world_size
        if state.is_main:
            _json(
                {
                    "backend": backend["active_backend"],
                    "batches": state.world_size if state.enabled else 1,
                    "distributed": state.report(),
                    "loss": float(loss.cpu()),
                    "precision": precision,
                }
...
def _infer(args) -> int:
    ...
    try:
        ...
        if state.enabled:
            gathered = [torch.empty_like(local_output) for _ in range(state.world_size)]
            torch_dist.all_gather(gathered, local_output)
...
def _export(args) -> int:
    if args.checkpoint_mode == "portable":
        model = load_dabsn(args.checkpoint, map_location="cpu")
...
def _add_execution_arguments(command: argparse.ArgumentParser) -> None:
    ...
    command.add_argument("--distributed", choices=("none", "ddp", "fsdp"), default="none")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_command = commands.add_parser("train", help="train a new model from config and prepared data")
    _add_training_arguments(train_command, config_required=False)
    train_command.add_argument("--resume", action="store_true", help="continue the same model, optimizer, and step")
    train_command.set_defaults(handler=_train, training_kind="train")

    finetune_command = commands.add_parser("finetune", help="start a new optimizer from existing model weights")
    _add_training_arguments(finetune_command, config_required=False)
    finetune_command.add_argument("--checkpoint", required=True, help="portable SafeTensors model to initialize from")
    finetune_command.set_defaults(handler=_finetune, resume=False, training_kind="finetune")

    pretrain_command = commands.add_parser("pretrain", help="next-token pretraining from an mmap token corpus")
    ...
    pretrain_command.add_argument("--backend", choices=("auto", "reference", "cpu", "cuda"), default="auto")
    pretrain_command.add_argument("--distributed", choices=("none", "ddp", "fsdp"))
    pretrain_command.add_argument("--precision", choices=("auto", "fp32", "fp16", "bf16"))
    pretrain_command.add_argument("--checkpoint-mode", choices=("portable", "sharded"), default="portable")
    ...
    evaluate_command = commands.add_parser("evaluate", help="evaluate a portable or sharded checkpoint")
    evaluate_command.add_argument("--checkpoint", required=True)
    evaluate_command.add_argument("--checkpoint-mode", choices=("portable", "sharded"), default="portable")
    ...
    infer_command = commands.add_parser("infer", help="run inference from a portable or sharded checkpoint")
    infer_command.add_argument("--checkpoint", required=True)
    infer_command.add_argument("--checkpoint-mode", choices=("portable", "sharded"), default="portable")
    ...
    export_command = commands.add_parser("export", help="export SafeTensors weights or a Torch program")
    export_command.add_argument("--checkpoint", required=True)
    export_command.add_argument("--checkpoint-mode", choices=("portable", "sharded"), default="portable")
    export_command.add_argument("--output", required=True)
    export_command.add_argument("--format", choices=("safetensors", "torch-export"), default="safetensors")
